 ...
 *
 * Internal-only endpoint for the Simulator UI to fire test webhook events
 * without needing a Razorpay signature. This endpoint:
 *   - Computes the correct HMAC SHA256 signature if RAZORPAY_WEBHOOK_SECRET is set
 *   - Then calls the main /api/webhooks handler logic directly (no HTTP round-trip)
 *
 * This ensures the simulator works correctly regardless of whether Razorpay
 * credentials are configured.
 *
 * DO NOT expose this endpoint publicly in production — it accepts unsigned payloads.
 * Security is enforced by only processing events from the local Next.js server process.
 */
...
export async function POST(request) {
  try {
    ...
    // Extract entities from payload (same logic as /api/webhooks)
    ...
    // Determine / auto-create customer (same as webhook handler)
