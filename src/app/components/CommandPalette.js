...
export function CommandPalette({ isOpen, onClose }) {
  ...
  const staticActions = [
    { id: 'nav-overview', label: 'Go to Overview', category: 'Navigation', icon: IconDashboard, run: () => router.push('/') },
    ...
    <div className="modal-overlay" onClick={onClose} role="dialog" aria-modal="true">
      <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '560px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px 20px', borderBottom: '1px solid var(--glass-border)' }}>
          <IconSearch size={18} color="var(--primary-glow)" />
          <input
            ref={inputRef}
            type="text"
            className="input"
          ...
          />
          <span style={{ fontSize: '11px', color: 'var(--text-dim)', fontFamily: 'monospace' }}>ESC</span>
        </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    background: idx === selectedIndex ? 'rgba(37, 99, 235, 0.12)' : 'transparent',
                    color: idx === selectedIndex ? '#ffffff' : 'var(--text-primary)',
                    transition: 'background var(--transition-fast)'
                  }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <span style={{ color: idx === selectedIndex ? '#60a5fa' : 'var(--text-muted)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 18px', borderTop: '1px solid var(--glass-border)', background: 'rgba(8, 10, 15, 0.4)', fontSize: '11px', color: 'var(--text-dim)' }}>
          <div>Use <kbd style={{ padding: '1px 4px', background: 'rgba(255,255,255,0.06)', borderRadius: '3px' }}>↑</kbd> <kbd style={{ padding: '1px 4px', background: 'rgba(255,255,255,0.06)', borderRadius: '3px' }}>↓</kbd> to navigate, <kbd style={{ padding: '1px 4px', background: 'rgba(255,255,255,0.06)', borderRadius: '3px' }}>↵</kbd> to select</div>
